<template>
    <section dir="rtl" id="cta" class="bg-brightRed ">
        <div class="container mx-auto p-10 flex flex-col space-y-6 md:flex-row md:justify-around items-center ">
            <h1 class="text-white md:w-3/5 text-center md:text-left text-4xl">
                .امنحنا الفرصة، لنصنع النجاح
            </h1>
            <div class="md:w-2/5 flex justify-end">
                <a href="#contact-from" class="px-6  py-2 w-fit text-brightRed bg-white rounded-full hover:bg-gray-300">
                    لنعمل سويا
                </a>
            </div>
        </div>
    </section>
</template>

<script setup lang="ts">

</script>

<style scoped></style>